...
begin
  ...
  try
    ...
    Results.Add('Дата прохождения: ' + FormatDateTime('dd-mm-yy', FStartTime));
    Results.Add('Начало прохождения: ' + FormatDateTime('hh:nn:ss', FStartTime));
    Results.Add('Оконччание прохождения: ' + FormatDateTime('hh:nn:ss', FEndTime));
    Results.Add('Время прохождения: ' + IntToStr(FDuration.Minutes) + ' мин ' + IntToStr(FDuration.Seconds) + ' сек');
    Results.Add('---');
...
begin
  ...
  FDuration := TTimeSpan.FromMilliseconds(MillisecondsDiff);
end;
